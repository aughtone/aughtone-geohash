...
import com.github.aughtone.geohash.support.lowerNibble
...
object Geohash {
     ...
     */
    private val NEIGHBOURS: Map<Direction, Map<Parity, String>> = mapOf(
     ...
     */
    private val BORDERS: Map<Direction, Map<Parity, String>> = mapOf(
        Direction.BOTTOM to mapOf(
            Parity.EVEN to "028b",
     ...
     */
    fun adjacentHash(hash: String, direction: Direction): String {
     ...
     * around 180 degrees.
     */
    private fun adjacentHashAtBorder(hash: String, direction: Direction): String? {
        ...
        when (direction) {
            Direction.BOTTOM -> if (abs(centre.latitude - widthDegrees(hash.length) / 2 + 90) < PRECISION) {
                return encodeGeohash(centre.latitude, centre.longitude + 180, hash.length)
            }

            Direction.TOP -> if (abs(centre.latitude + widthDegrees(hash.length) / 2 - 90) < PRECISION) {
                return encodeGeohash(centre.latitude, centre.longitude + 180, hash.length)
            }

            Direction.LEFT -> if (abs(centre.longitude - widthDegrees(hash.length) / 2 + 180) < PRECISION) {
                return encodeGeohash(centre.latitude, 180.0, hash.length)
            }

            Direction.RIGHT -> if (abs(centre.longitude + widthDegrees(hash.length) / 2 - 180) < PRECISION) {
                return encodeGeohash(centre.latitude, -180.0, hash.length)
            }
        }
     ...
     */
    fun adjacentHash(hash: String, direction: Direction, steps: Int): String {
    ...
    fun neighbours(hash: String): List<String> {
        val left = adjacentHash(hash, Direction.LEFT)
        val right = adjacentHash(hash, Direction.RIGHT)
        return listOf(
            left,
            right,
            adjacentHash(hash, Direction.TOP),
            adjacentHash(hash, Direction.BOTTOM),
            adjacentHash(left, Direction.TOP),
            adjacentHash(left, Direction.BOTTOM),
            adjacentHash(right, Direction.TOP),
            adjacentHash(right, Direction.BOTTOM),
        )
    }
    ...
    fun hashContains(geohash: String, latitude: Double, longitude: Double): Boolean {
        ...
        return abs(centre.latitude - latitude) <= heightDegrees(geohash.length) / 2
                && abs(to180(centre.longitude - longitude)) <= widthDegrees(geohash.length) / 2
    }
    ...
    fun coverBoundingBoxMaxHashes(
        ...
        var coverageL: CoverageLongs? = null
        for (length in startLength..MAX_HASH_LENGTH) {
            val cl: CoverageLongs = coverBoundingBoxLongs(
                topLeftLat, topLeftLon, bottomRightLat,
    ...
    fun coverBoundingBoxLongs(
        ...
        run {
            ...
            while (lat <= topLeftLat) {
                ...
                while (lon <= newBottomRightLon) {
                    hashes.add(encodeToLong(lat, to180(lon), length))
        ...
        var lat = bottomRightLat
        while (lat <= topLeftLat) {
            hashes.add(encodeToLong(lat, to180(newBottomRightLon), length))
            lat += actualHeightDegreesPerHash
        }
        var lon = newTopLeftLon
        while (lon <= newBottomRightLon) {
            hashes.add(encodeToLong(topLeftLat, to180(lon), length))
            lon += actualWidthDegreesPerHash
        }
        // ensure that the topRight corner is covered
        hashes.add(encodeToLong(topLeftLat, to180(newBottomRightLon), length))
        ...
        return CoverageLongs(hashes.toLongArray(), hashes.count(), ratio)
    }
    ...
    fun heightDegrees(n: Int): Double {
        return if (n > MAX_HASH_LENGTH) calculateHeightDegrees(
            n
        )
    ...
    fun widthDegrees(n: Int): Double {
        return if (n > MAX_HASH_LENGTH) calculateWidthDegrees(
            n
        )
        ...
        val buffer = StringBuilder().apply {
            for (bottom in fromBottom..toBottom) {
                for (right in fromRight..toRight) {
                    var h = adjacentHash(hash, Direction.RIGHT, right)
    ...
    fun encodeGeohash(latitude: Double, longitude: Double, length: Int = MAX_HASH_LENGTH): String {
        ...
        return fromLongToString(encodeToLong(latitude, to180(longitude), length))
    }
